...
my $usage = "$0 country_group_file data_table [scatter, animated_scatter, residuals, group_residuals]";
...
if ( $ARGV[2] eq 'scatter' ) {
	&drawChartScatter();
} elsif ($ARGV[2] eq 'animated_scatter') {
	&drawChartAnimatedScatter();
    ...
    foreach my $group (@groups_to_plot) {
    	foreach my $country ( keys %{$country_grps{$group}} ) {
    		if ( $data{$country}->{use} ) {
    			...
    			my $date_label = $dates[$max]->monname . " " . $dates[$max]->year;
    			my $date_string .= "\"$country ($date_label)\",";
				...
				my $x = $data{$country}->{use}->[$max];
				my $y = $data{$country}->{provide}->[$max];
				$data_string .= "{x: $x, y: $y}";
...
<div class="header">
	...
	DSI provision and use values for each country on $end_year.</p>
</div>
<div class="row">
	<div class="column-main" style="width: 9%; float:left"></div>
	...
	<div class="column-side" style="width: 9%; float:left"></div>
</div>
...
<script>
	...
	const config = {
	  ...
	  options: {
		...
		elements: { point: { hitRadius: 10, radius: 4 } },
		scales: {
		  x: {
			 type: 'linear',
			 position: 'bottom',
